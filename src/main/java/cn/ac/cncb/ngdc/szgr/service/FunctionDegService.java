...
import cn.ac.cncb.ngdc.szgr.dao.FunctionDegDAO;
...
@Service
public class FunctionDegService {
    @Autowired
    FunctionDegDAO functionDegDAO;

    public Result queryFuncDeg(Integer pageSize, Integer pageIndex, String gene, String sort_field, String sort_direction){
        Long total = 0L;
        List<FunctionDeg> data = null;
        Meta meta = new Meta();
        Specification<FunctionDeg> queryCondition = new Specification<FunctionDeg>(){
            @Override
            public Predicate toPredicate(Root<FunctionDeg> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
        ...
        try {
            data = functionDegDAO.findAll(queryCondition, PageRequest.of(pageIndex, pageSize)).getContent();

            total = functionDegDAO.count(queryCondition);
