...
import cn.ac.cncb.ngdc.szgr.dao.GSEADAO;
...
@Service
public class GSEAService {
    @Autowired
    GSEADAO gseaDAO;

    public Result queryGSEA(Integer pageSize, Integer pageIndex, String gene, String sort_field, String sort_direction){
        Long total = 0L;
        List<GSEA> data = null;
        Meta meta = new Meta();
        Specification<GSEA> queryCondition = new Specification<GSEA>(){
            @Override
            public Predicate toPredicate(Root<GSEA> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
        ...
        try {
            data = gseaDAO.findAll(queryCondition, PageRequest.of(pageIndex, pageSize)).getContent();
            if (gene != null ){
                total = 1L;
            }else {
                total = gseaDAO.count(queryCondition);
            }
            meta.setTotal(total);
